...
import kithyLesssons.simpleoperations.KitcheDAO;
import kithyLesssons.simpleoperations.KitchenRepository;
import kithyLesssons.simpleoperations.KitchenService;
import org.springframework.data.crossstore.ChangeSetPersister;

public class Test {

    public static void main(String[] args) throws ChangeSetPersister.NotFoundException {
        KitchenRepository repository = new KitcheDAO();
        KitchenService service = new KitchenService(repository);

        service.getDishesByType()
               .forEach((type, dishes) -> System.out.println("dish type: " + type + " dishes: " + dishes));
//
...
//               .map(c -> c.chars()
//                          .mapToObj(x -> (char) x))
//               .flatMap(p -> p.filter(Character::isDigit))
//               .forEach(System.out::println);
//
//        strings.stream()
...
//        System.out.println(service.returnMapWithMapWithList());

        System.out.println("DEMO: " + service.flatMappingKitchenNameToDish());
//        System.out.println(service.numbersExample());
//        System.out.println(service.getIntegerLongMap());
