...
public class Chapter3Challenge1 {
    ...
    public static void main(String[] args) {
        ...
        List<String> collect = people.stream().map(x -> x.name).collect(Collectors.toList());
        ...
        cars.stream().filter(color -> color.color.equals("blue")).collect(Collectors.toList());
        ...
        Float sumOfSalary = employees.stream().map(salaries -> salaries.salary).reduce(0f, Float::sum);
        ...
        Map<String, Float> averageSalary = employees
                ...
                .collect(Collectors.toMap(
                        ...
                        entry -> entry.getValue()
                                .stream()
                                .map(k -> k.salary)
                                .reduce(0f, (acc, y) -> acc + y) / entry.getValue().size()));
